...
import { usePodcast } from '@/store/podcastStore';
...
export default function Podcast() {
  const { categories, episodes, addCategory, removeCategory, addEpisode, deleteEpisode } =
    usePodcast();
  ...
  const onGenerate = async () => {
    ...
    setGenerating(true);
    try {
      const result = await generatePodcastScript({ categories, targetMinutes: 4 });
      addEpisode({
        id: String(Date.now()),
  ...
  return (
    <ApiKeyGate require={['gemini', 'newsapi']}>
                  ...
                  <Space style={{ width: '100%', justifyContent: 'space-between' }} wrap>
                    <Space direction="vertical" size={2}>
                      <Typography.Text strong>
                        {dayjs(ep.id, 'x').format('DD MMM YYYY · HH:mm')}
                      </Typography.Text>
                      <Space wrap size={4}>
                        {ep.categories.map((c) => (
